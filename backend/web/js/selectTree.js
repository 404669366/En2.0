...
configFun.prototype = {
        ...
        if(self.length == 0 && res.dataHover.pid != 0){
            ...
            if(res.type == 1){
                ...
                radioFather[0].onclick = function(){
                    ...
                    var radioFather_father = radioFather;
                    var radioList = radioFather.parents('.f-treeList-title').eq(0).parents('.f-treeList-top').eq(0).parents('.f-treeList').eq(0);
            ...
            //查找所有的单独子级
            var topDom = selfDom.parents('.f-treeListOne').eq(0);
            var oneDom = topDom.parents('.f-treeListWrapper').eq(0);
            var treelist = oneDom.find('.f-treeList');
            for(var i=0; i<treelist.length; i++){
                if(treelist.eq(i).siblings().length == 1){
                    var endLsit = treelist.eq(i).children('.f-treeList-top').eq(0).children('.f-treeList-lineEnd');
                    if(endLsit.length == 0){
                        (function(k){
                            var hoverList = treelist.eq(k);
                            var radio = hoverList.find('.f-treeList-radio').eq(0);
                            var old = hoverList.find('.f-treeList-old').eq(0);
                            if(res.type == 1){
                                radio[0].onclick = function(){
                                    var radio_icon = $(this).children('span').eq(0);
                                    var radio_class = radio_icon.attr('class');
                                    var fatherWrapper = $(this).parents('.f-treeListOne').eq(0);
                                    var radioZi = fatherWrapper.find('.f-treeList-radio');
                                    radio_icon.removeClass(radio_icon.attr('class'));
                                    if(radio_class == 'f-iconRadio'){
                                        for(var i=0; i<radioZi.length; i++){
                                            var radioZi_icon = radioZi.eq(i).children('span').eq(0);
                                            radioZi_icon.removeClass(radioZi_icon.attr('class'));
                                            radioZi_icon.addClass('f-iconRadioTrue');
                                        };
                                    }else{
                                        for(var i=0; i<radioZi.length; i++){
                                            var radioZi_icon = radioZi.eq(i).children('span').eq(0);
                                            radioZi_icon.removeClass(radioZi_icon.attr('class'));
                                            radioZi_icon.addClass('f-iconRadio');
                                        };
                                    };
                                };
                            };
                            old[0].onclick = function(){
                                var radio_icon = $(this).siblings('.f-treeList-title').eq(0).children('.f-treeList-radio').eq(0).children('span').eq(0);
                                var radio_class = radio_icon.attr('class');
                                var fatherWrapper = $(this).parents('.f-treeListOne').eq(0);
                                var radioZi = fatherWrapper.find('.f-treeList-radio');
                                if($(this).attr('class').indexOf('f-on') == -1){
                                    var fatherOld = $(this);
                                    var nextWrapper = fatherOld.parents('.f-treeList').eq(0).children('.f-treeListWrapper').eq(0);
                                    var iconDiv = fatherOld.children('span').eq(0);
                                    var flagClass = iconDiv.attr('class');
                                    if(flagClass == 'f-iconJia'){
                                        iconDiv.removeClass(flagClass);
                                        iconDiv.addClass('f-iconJian');
                                        nextWrapper.removeClass('hide');
                                        nextWrapper.addClass('show');
                                        //如选中则子级选中
                                        /*if(radio_class == 'f-iconRadioTrue'){
                                            for(var i=0; i<radioZi.length; i++){
                                                var radioZi_icon = radioZi.eq(i).children('span').eq(0);
                                                radioZi_icon.removeClass(radioZi_icon.attr('class'));
                                                radioZi_icon.addClass('f-iconRadioTrue');
                                    ...
                                    if(flagClass == 'f-iconJian'){
                                        iconDiv.removeClass(flagClass);
                                        iconDiv.addClass('f-iconJia');
                                        nextWrapper.removeClass('show');
                                        nextWrapper.addClass('hide');
                                    };
                                };
                            }
                        })(i)
    ...
    fatherRadio_true:function(res){
        var t = this;
        var fatherList = res.parents('.f-treeList').eq(0);
        var fatherList_radio = fatherList.find('.f-treeList-radio').eq(0).find('span').eq(0);
        fatherList_radio.removeClass(fatherList_radio.attr('class'));
        fatherList_radio.addClass('f-iconRadioTrue');
        if(fatherList.attr('class').indexOf('f-treeListOne') == -1){
            t.fatherRadio_true(fatherList)
        };
    },
    fatherRadio_false:function(res){
        var t = this;
        var fatherList = res.parents('.f-treeList').eq(0);
        var fatherList_radio = fatherList.find('.f-treeList-radio').eq(0).find('span').eq(0);
        var sibList = res.siblings('.f-treeList');
        var num =0;
        for(var i=0; i<sibList.length; i++){
            var sibList_radio = sibList.eq(i).find('.f-treeList-radio').eq(0).find('span').eq(0);
            if(sibList_radio.attr('class') == 'f-iconRadio'){
                num++;
            };
        };
        if(num == sibList.length){
            fatherList_radio.removeClass(fatherList_radio.attr('class'));
            fatherList_radio.addClass('f-iconRadio');
        };
        if(fatherList.attr('class').indexOf('f-treeListOne') == -1){
            t.fatherRadio_false(fatherList)
        };
    }
};
var selectTree = new configFun();
