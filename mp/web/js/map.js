...
document.write('<style>.anchorBL{display: none;}</style>');
window.TILE_VERSION = {
    "ditu": {
...
function map(config) {
    var zoom = config.zoom || 16;
    config.addressName = config.addressName || 'address';
    config.placeholder = config.placeholder || '';
    config.lngName = config.lngName || 'lng';
    config.latName = config.latName || 'lat';
    if (config.readOnly) {
        config.addressName = config.lngName = config.latName = '';
    }
    $('.' + config.element)
        .html('')
        .append('<input class="address" placeholder="' + config.placeholder + '" id="' + config.element + 'Address" type="text" name="' + config.addressName + '"/>')
        ...
        .append('<style>.' + config.element + '{' + (config.style || 'width: 100%;height: 20rem;') + '}.map{width:calc(100% - 1px);height:91%;}.address{height: 9%;line-height: 9%;width: 100%;display:inline-block;box-sizing: border-box;padding: 0 0.5rem;font-size: ' + (config.fontSize || '1.2rem') + '}.tangram-suggestion-main{padding-left: 1px;}</style>');
    ...
    var ac = new BMap.Autocomplete({
        input: config.element + 'Address',
        location: map
    });

    ac.addEventListener("onconfirm", function (e) {
        var re = e.item.value;
        var address = re.province + re.city + re.district + re.street + re.business;
    ...
    if (!config.default) {
        ...
        geol.getCurrentPosition(function (r) {
        ...
        if (config.default.address && config.default.lng && config.default.lat) {
            ac.setInputValue(config.default.address);
            $('.' + config.element).find('.lng').val(config.default.lng);
            $('.' + config.element).find('.lat').val(config.default.lat);
            var defPoint = new BMap.Point(config.default.lng, config.default.lat);
            map.centerAndZoom(defPoint, zoom);
            map.addOverlay(new BMap.Marker(defPoint));
            map.panTo(defPoint);
        } else {
            var geol = new BMap.Geolocation();
            geol.getCurrentPosition(function (r) {
    ...
    function outVal(point, needAddress) {
        zoom = map.getZoom();
        $('.' + config.element).find('.lng').val(point.lng);
        $('.' + config.element).find('.lat').val(point.lat);
        if (needAddress) {
            geoc.getLocation(point, function (rs) {
                $('.' + config.element).find('.address').val(getAddress(point, rs));
            });
        }
    }

    function getAddress(point, result) {
        if (result.surroundingPois.length) {
            var obj = result.surroundingPois[0];
            $.each(result.surroundingPois, function (k, v) {
                if (getFlatternDistance(point.lat, point.lng, v.point.lat, v.point.lng) < getFlatternDistance(point.lat, point.lng, obj.point.lat, obj.point.lng)) {
                    obj = v;
                }
            });
            return obj.address + ' ' + obj.title;
        }
