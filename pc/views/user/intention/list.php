...
<div class="center">
    <div class="box">
        <ul class="menu">
            <li onselectstart="return false" class="active" data-url="/user/intention/list.html">我的认购</li>
            ...
            <?php if ($data): ?>
                <?php foreach ($data as $v): ?>
                    <div class="one">
                        ...
                        <div class="btns">
                        <span>
                            <?php if ($v['status'] == '待付定金' && $v['fStatus'] == '正在融资'): ?>
                                <a class="btn" href="/user/intention/info.html?id=<?= $v['id'] ?>">前往付款</a>
                            <?php endif; ?>
                            <?php if ($v['status'] == '待付定金' && $v['fStatus'] == '融资完成'): ?>
                                <a class="btn" href="/user/intention/del.html?id=<?= $v['id'] ?>">删除</a>
                            <?php endif; ?>
                            <?php if ($v['status'] == '已付定金'): ?>
                                <?php if (time() <= ($v['pay_at'] + \vendor\project\helpers\Constant::orderBackTime())): ?>
                                    <a class="btn" href="/user/intention/back.html?id=<?= $v['id'] ?>">申请退款</a>
                                <?php endif; ?>
                                <?php if ($v['fStatus'] == '融资完成'): ?>
                                    <a class="btn" href="/user/intention/upload.html?id=<?= $v['id'] ?>">上传凭证</a>
                                <?php endif; ?>
                            <?php endif; ?>
                            <?php if ($v['status'] == '申请退款'): ?>
                                <a class="btn" href="/user/intention/revoke.html?id=<?= $v['id'] ?>">撤销退款</a>
                            <?php endif; ?>
                            <?php if ($v['status'] == '审核不通过'): ?>
                                <a class="btn" href="/user/intention/upload.html?id=<?= $v['id'] ?>">上传凭证</a>
                            <?php endif; ?>
                            <?php if ($v['status'] == '已退款'): ?>
                                <a class="btn" href="/user/intention/del.html?id=<?= $v['id'] ?>">删除</a>
